				...
				<div class="mui-card-footer">
                   <div class="price">
                    ...
                    <div class="mui-pull-right">
                        <router-link :to="'/limitShopSetailC/'+item.product_id+'/'+item.seckill_id" tag="button" type="button" class="mui-btn mui-btn-danger" v-if="item.seckill_status == 1">立即抢购</router-link>
                        <button type="button" class="mui-btn mui-btn-gray" v-else-if="item.iseckill_status != 1">立即抢购</button>
                    </div>
                </div>
			</div>
        </div>                 
...
<script>
    ...
    import { b2cTimeLimit } from '../../../../api/index.js'
    export default {
        data() {
            return {
                titmeLimit:[],//限时数据列表
