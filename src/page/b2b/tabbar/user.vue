								...
								<ul class="mui-table-view">
									<li class="mui-table-view-cell" v-for="(item,index) in orderListAll" :key="index">
                                        ...
                                        <div class="order-details-content mui-clearfix">
                                            <router-link  :to="'/userb/detailsB/'+item.order_no" tag="div" class="contain" v-for="itemProduct in item.product" :key="itemProduct.id">
                                                <div class="content-left">
                                                    <img :src=" itemProduct.thumb" class="img-big" alt="">
                                                </div>
                                                <div class="content-right">
                                                    <div class="content-right-text">{{ itemProduct.title }}</div>
                                                    <div class="productAttr"><span v-if="itemProduct.product_attr != 0">{{ itemProduct.product_attr }}</span></div>
                                                    <div class="pricec"><span class="right-red">￥{{ itemProduct.price }}</span><span class="right-grey">x{{ itemProduct.num }}</span></div>
                                                </div>
                                            </router-link> 
                                            ...
                                            <div class="confirm">
                                                <!-- 待收货  ===> 4 -->
                                                <router-link :to="'/userb/ReturnShopBList/' +item.order_no " tag="span" class="wu"  v-if="item.order_status == 4 || item.order_status == 3 ? true : false">申请退款</router-link>
                                                <span class="shou" v-if="item.order_status == 4 ? true : false" @click="orderSure(item.order_no)">确认收货</span>
                                                <!-- 退款和已完成的状态有查看详情 ===> 6 -->
                                                <router-link :to="'/userb/detailsB/' +item.order_id " tag="span" class="wu"  v-if=" item.order_status == 1 ? true : false" >查看详情</router-link>
                                                <!-- 待付款 ===> 2 -->
                                                ...
                                                <!-- 待评价 ===> 5 -->
                                                <router-link :to="'/userb/evaluationB/EvaluationListB/'+item.order_no" tag="span" class="shou" v-if="item.order_status == 5 ? true : false">评价</router-link>
                                                <!-- 待发货 ===> 3 -->
                                                ...
                                                <!-- 退货退款===> 6 -->
                                                <span  class="wu" v-if="item.order_status == 6 && item.refund_status != 1? true : false" @click="closeReturn(item.order_no,item.detail_id)">撤销申请</span>
                                                <router-link :to="'/userb/pricedetailcB/'+item.order_no+'/'+item.detail_id" tag="span"  class="shou" v-if="item.order_status == 6 ? true : false" >查看详情</router-link>
                                            </div>
                                        </div>
									</li>
								</ul>
...
import {  orderNum, b2bOrder, b2bOrderSure, b2bOrderConcal, b2bOrderDelete, b2bOrderWarn ,CloseReturnShop,b2bAliPay,b2bWechatPay} from '../../../api/index.js'
export default {
    inject :['reload'],
    data() {
        return {
            status: 0, //订单状态 ，需要传递的参数
            limit: 10, //每页传多少数据，可不传，默认为10
            page: 0,   //页码，可不传，默认为0
            orderType:1,//订单类型：1：B2B；2：B2C
    ...
    methods:{
        ...
        // 确认订单按钮
        orderSure( orderId ){
            let params = {order_no : orderId,type:this.orderType }
