    ...
    <div class="info">
        ...
        <div class="mui-content">
            ...
            <ul class="mui-table-view">
                ...
                <router-link to="/defaultAddress" tag="li" class="mui-table-view-cell"  v-else-if="userData !== null && addressList == null">
                ...
                <router-link to="/defaultAddress" tag="li" class="mui-table-view-cell" v-else>
                    <a class="selectedA mui-navigate-right">
                        <p class="contact">{{addressList.name}}:{{addressList.mobile}}</p>
                        <p class="contactAddress mui-ellipsis-2"><span class="mui-icon mui-icon-location mui-pull-left"></span>{{ userData.area}}{{addressList.address}}</p>
                    </a>
                </router-link>
            </ul>
            <!-- 详情 -->
                ...
                <ul class="mui-table-view mui-table-Info">
                    <li class="mui-table-view-cell" v-for="item in dataList" :key="item.shop_id">
                        ...
                        <ul class="mui-table-view  moneyInfo">
                            <li class="mui-table-view-cell">
                                <a>
                                    商品金额: <span>{{ item.total_price }}</span>
                                </a>
                            </li>
                        </ul>
                    </li>
...
import mui from '../../../../static/mui/js/mui.min.js'
    import { userInfo, b2bConfirmOrder, b2bSubmitCart , b2bAliPay ,b2bWechatPay } from '../../../api/index.js'
    export default {
        data() {
            return {
        ...
        methods:{
            ...
            getData(){
                ...
                b2bConfirmOrder(params).then(res=>{
                    that.dataList = res.data
                    that.dataList.forEach(item=>{
                        that.allTotalPrice += item.total_price
                    })
                })
            },
            // 如果没有选择收货地址不能提交订单
