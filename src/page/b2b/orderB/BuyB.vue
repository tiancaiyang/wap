    ...
    <div class="info">
        ...
        <div class="mui-content">
            ...
            <!-- 备注类 -->
             <ul class="mui-table-view otherInfo">
                ...
                <li class="mui-table-view-cell">
                    <a>
                        买家留言
                        <textarea  placeholder="选填,可填写您和卖家保持一致" v-model="remarks"></textarea>
                    </a>
                </li>
            </ul>
            <!-- 商品金额和运费 -->
            <ul class="mui-table-view moneyInfo">
                <li class="mui-table-view-cell">
                    <a>商品金额 
                        <span class="mui-pull-right" v-if="dataList.product_attr == '' && dataList.activity_type == 0">￥{{ dataList.price }}</span>
                        <span class="mui-pull-right" v-else-if="dataList.product_attr == '' && dataList.activity_type == 1">￥{{ dataList.priceDiscount }}</span>
                        <span class="mui-pull-right" v-else-if="dataList.activity_type == 2">￥{{ dataList.seckill_price }}</span>
                        <span class="mui-pull-right" v-else-if="dataList.product_attr !== ''">￥{{ dataList.selectPrice }}</span>
                    </a>
                </li>
            </ul>    
        </div>
        ...
        <div class="footer">
            <a class="allMoner">合计金额:
                <span>￥</span><span ref="priceTotal" v-if="dataList.product_attr == '' && dataList.activity_type == 0" >{{ dataList.price * dataList.num | strip}}</span>
                               <span ref="priceTotal" v-else-if="dataList.product_attr == '' && dataList.activity_type == 1" >{{ dataList.priceDiscount * dataList.num | strip}}</span>
                               <span ref="priceTotal" v-else-if="dataList.activity_type == 2" >{{ dataList.seckill_price * dataList.num | strip}}</span>
                               <span ref="priceTotal" v-else-if="dataList.product_attr !== ''" >{{ dataList.selectPrice * dataList.num | strip}}</span>
            </a>
...
<script>
    import { userInfo, IndexAddress , b2bSubmitOrder , b2bAliPay, b2bWechatPay} from '../../../api/index.js'
    ...
    export default {
        ...
        methods:{
            ...
            surePayOrder(){
                ...
                // 接收地址id
                if(that.addressList){
                    that.addressId = that.addressList.address_id
                }else{
                ...
                let params = {
                    ...
                    brand_member_pos_id: that.dataList.brand_member_pos_id,
                    limit_price: that.dataList.limit_price,
                    comment: that.remarks
                }
                b2bSubmitOrder(params).then(res=>{
                    if(res.code !== 0){
